...
compilers. */
const static float cie_colour_match[81][3] PROGMEM = {
...
void wavelength_to_xyz(int wavelength, float *x, float *y, float *z) {
    int lambda;
    float X, Y, Z, XYZ = 0;
    if (wavelength < 380) {
        lambda = 380;
    ...
    X = cie_colour_match[(lambda - 380)/5][0];
    Y = cie_colour_match[(lambda - 380)/5][1];
    Z = cie_colour_match[(lambda - 380)/5][2];
    XYZ = X + Y + Z;
    *x = X / XYZ;
    *y = Y / XYZ;
    *z = Z / XYZ;
...
/*  Simple wrapper. Added by David Strickland in April 2015
    for use in an Arduino color organ.
    https://github.com/d-strickland/arduino-projects
*/
void wavelength_to_rgb(int wavelength, float *r, float *g, float *b)
{
    float x, y, z;
    wavelength_to_xyz(wavelength, &x, &y, &z);
...
{ 
    ...
    printf("Temperature      R     G     B\n");
    printf("-----------    ----- ----- -----\n");
    
    for (t = 1000; t <= 10000; t+= 500) {
        spectrum_to_rgb(t, &re, &gr, &bl);
        printf("  %5.0f K      %.3f %.3f %.3f\n", t, re, gr, bl);
    }
    return 0;
} */
